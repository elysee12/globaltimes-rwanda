...
const Education = () => {
  ...
  const educationArticles = useMemo(() => {
    return articles.filter(a => a.category.toLowerCase() === 'education');
  }, [articles]);
  ...
  }, [educationArticles, translatedData]);

  const topStories = useMemo(() => {
    return educationNews.filter(news => news.featured || news.trending).slice(0, 3);
  }, [educationNews]);

  const latestNews = useMemo(() => {
    return educationNews.filter(news => !news.featured && !news.trending);
  }, [educationNews]);
