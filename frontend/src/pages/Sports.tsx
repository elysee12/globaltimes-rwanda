...
const Sports = () => {
  ...
  const sportsArticles = useMemo(() => {
    return articles.filter(a => a.category.toLowerCase() === 'sports');
  }, [articles]);
  ...
  }, [sportsArticles, translatedData]);

  const topStories = useMemo(() => {
    return sportsNews.filter(news => news.featured || news.trending).slice(0, 3);
  }, [sportsNews]);

  const latestNews = useMemo(() => {
    return sportsNews.filter(news => !news.featured && !news.trending);
  }, [sportsNews]);
