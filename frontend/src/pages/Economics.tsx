...
const Economics = () => {
  ...
  const economicsArticles = useMemo(() => {
    return articles.filter(a => a.category.toLowerCase() === 'economics');
  }, [articles]);

  useEffect(() => {
    const translateArticles = async () => {
      const translations = new Map();
      for (const article of economicsArticles) {
        try {
    ...
    if (economicsArticles.length > 0) {
      translateArticles();
    }
  }, [economicsArticles, language]);

  const economicsNews = useMemo(() => {
    return economicsArticles.map(article => {
  ...
  }, [economicsArticles, translatedData]);

  const topStories = useMemo(() => {
    return economicsNews.filter(news => news.featured || news.trending).slice(0, 3);
  }, [economicsNews]);

  const latestNews = useMemo(() => {
    return economicsNews.filter(news => !news.featured && !news.trending);
  }, [economicsNews]);
