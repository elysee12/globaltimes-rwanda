...
const Entertainment = () => {
  ...
  const entertainmentArticles = useMemo(() => {
    return articles.filter(a => a.category.toLowerCase() === 'entertainment');
  }, [articles]);

  useEffect(() => {
    const translateArticles = async () => {
      const translations = new Map();
      for (const article of entertainmentArticles) {
        try {
  ...
  }, [entertainmentArticles, language]);

  const entertainmentNews = useMemo(() => {
    return entertainmentArticles.map(article => {
  ...
  }, [entertainmentArticles, translatedData]);

  const topStories = useMemo(() => {
    return entertainmentNews.filter(news => news.featured || news.trending).slice(0, 3);
  }, [entertainmentNews]);

  const latestNews = useMemo(() => {
    return entertainmentNews.filter(news => !news.featured && !news.trending);
  }, [entertainmentNews]);
