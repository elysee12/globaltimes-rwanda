...
const MyStory = () => {
  ...
  const storyArticles = useMemo(() => {
    return articles.filter((a) => a.category.toLowerCase() === "story");
  }, [articles]);
  ...
  }, [storyArticles, language]);

  const storyNews = useMemo(() => {
  ...
  }, [storyArticles, translatedData]);

  const topStories = useMemo(() => {
    return storyNews.filter((news) => news.featured || news.trending).slice(0, 3);
  }, [storyNews]);

  const latestNews = useMemo(() => {
    return storyNews.filter((news) => !news.featured && !news.trending);
  }, [storyNews]);
              ...
              <section>
                ...
                {latestNews.length > 0 ? (
                  ...
                  <p className="text-muted-foreground text-center py-8">
                    No story articles yet. Add some from the admin panel!
                  </p>
                )}
              </section>
            </>
