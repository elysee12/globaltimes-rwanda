...
const Business = () => {
  ...
  }, [businessArticles, language]);

  const businessNews = useMemo(() => {
    return businessArticles.map(article => {
  ...
  }, [businessArticles, translatedData]);

  const topStories = useMemo(() => {
    return businessNews.filter(news => news.featured || news.trending).slice(0, 3);
  }, [businessNews]);

  const latestNews = useMemo(() => {
    return businessNews.filter(news => !news.featured && !news.trending);
  }, [businessNews]);
